...
function parseTCGPlayerCSV(text) {
  ...
  if(headerFields.includes("tcgplayer id") || headerFields.includes("product name")) {
    ...
    for(let i = 1; i < lines.length; i++) {
      ...
      const { name, isFoil, variant } = extractNameMeta(rawName);
      ...
      // Derive Low/Mid/High from market price: Low=80%, Mid=market, High=120%
      const mid  = market;
      ...
      if(name) cards.push({qty, name, set, low, mid, high, isFoil, variant,
        condition, buyCost:null, source:"csv"});
    }
    return cards;
  }
  ...
  for(const line of dataLines) {
    ...
    const { name: cleanName, isFoil, variant } = extractNameMeta(name);
    if(cleanName) cards.push({qty, name:cleanName, set, low, mid, high,
      isFoil, variant, condition:"NM", buyCost:null, source:"csv"});
  }
  return cards;
}
