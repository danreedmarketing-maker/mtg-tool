...
export default function App() {
  ...
  // ── Derived values (declared before callbacks so all function bodies can reference them) ──
  ...
  const triggerCF = async (path, label) => {
    ...
    try {
      ...
      if(res.ok) setTimeout(() => { if(typeof gsReadPriceCache === "function") gsReadPriceCache(); }, 3000);
    } catch(e) {
  ...
  // Auto-load CF config when Sheet is connected
  useEffect(() => {
    if(gsToken && gsSheetId) loadCfConfig();
  }, [gsToken, gsSheetId]);
